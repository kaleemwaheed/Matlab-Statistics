...
% computing x[row] = (X[col]-mean1[i]) for every feature from x1 to x11 for Class1
% x = X-Xbar
...
for col = 1:11
    ...
    for row = 1:1500
        ...
        if trainLabels(row) == 1
            ...
            Matrix1(col,i) = trainX(row,col) - meanVec1(col);
            i=i+1;
           
            
        else
            
            Matrix0(col,j) = trainX(row,col) - meanVec0(col);
            j = j+1;
...
covMatrix1 = zeros(11,11);
covMatrix0 = zeros(11,11);
...
prior0 = PC0/1500;
prior1 = PC1/1500;
meanVecClass1 = meanVec1;
meanVecClass0 = meanVec0;
